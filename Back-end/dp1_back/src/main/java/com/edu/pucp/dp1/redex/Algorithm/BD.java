...
public class BD {
    ...
    public static void readFlights() {
        BD.flightsTemp = new ArrayList<>();
        try {
            File file = new File("Back-end/dp1_back/src/main/resources/input/planes_vuelo.v3.txt");
            Scanner scannerObj = new Scanner(file);
            ...
            while (scannerObj.hasNextLine()) {
                ...
                // Verificación de que la línea tiene el número correcto de partes
                if (split.length < 4 || split.length > 5) {
                    System.out.println("Línea con formato incorrecto: " + data);
                    continue; // Saltar esta línea
                }

                Flight flight = new Flight();
                flight.setCode(split[0] + "-" + split[1]);

                Airport departureAirport = null;
                Airport arrivalAirport = null;
                int index_system = 0;

                for (Airport airport : BD.airports) {
                    if (airport.getCode().equals(split[0])) {
                        departureAirport = airport;
                    }
                    if (airport.getCode().equals(split[1])) {
                        arrivalAirport = airport;
                    }
                    if (airport.getCode().equals("SPIM")) {
                        index_system = airport.getId();
                    }
                }

                if (departureAirport == null || arrivalAirport == null) {
                    throw new RuntimeException("Departure or Arrival airport not found");
                }
	...
	public static void readShipments(){
		...
		try {
			...
			while(scannerObj.hasNextLine()) {
		        ...
		        BD.shipmentsUnresolved.add(shipment);
			}
			scannerObj.close();
		}catch(Exception e) {
	...
	public static int read_list_shipment_with_date(long date_simulation, int type_simulation){
		...
		try {
			for (int j = 0; j < pack_files.size(); j++) {
		        ...
		        System.out.println("TAMANO DE LA LISTA DE ENVIOS: " + BD.shipmentsUnresolved.size());
			}
		}catch(Exception e) {
			System.out.println("EXCEPTION SHIPMENTS CON PARAMETRO: " + e.getMessage());
		}

		int tamanio=BD.shipmentsUnresolved.size();
	...
	public static void read_list_shipment_with_datePEDRO(long date_simulation){
		...
		System.out.println("TAMANO DE LA LISTA DE ENVIOS EN TOTAL: " + BD.shipmentsUnresolved.size());
	}
}
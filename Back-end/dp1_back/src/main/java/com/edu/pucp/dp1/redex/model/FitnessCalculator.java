...
public class FitnessCalculator {
    ...
    private boolean isOnTime(Package pkg, Flight flight) {
        // Implementar lógica para determinar si el paquete llega a su destino a tiempo
        return true; // Simplificación para el ejemplo
    }

    private Flight findFlightForPackage(Individual individual, Package pkg, List<Flight> flights) {
    ...
    private int countPackagesOnFlight(Individual individual, Flight flight, List<Package> packages) {
        // Implementar lógica para contar cuántos paquetes están asignados a un vuelo
        return 0; // Simplificación para el ejemplo
    }

    private double calculateTotalTransitTime(Individual individual, List<Package> packages, List<Flight> flights) {

          ...
          if (planePosition) {
            return (
              <MarkerF
                ...
                onMouseOver={() => setSelectedFlight(flight)}
                onMouseOut={() => setSelectedFlight(null)}
              />
            );
          }
          return null;
        })}
        {selectedFlight && selectedFlight.departure_airport && selectedFlight.arrival_airport && (
          <InfoWindow
            position={calculatePlanePosition(selectedFlight, tiempo_simulacion.dia_actual, tiempo_simulacion.tiempo_actual)}
            onCloseClick={() => setSelectedFlight(null)}
          >
            <div>
              <p>ID: {selectedFlight.id}</p>
              <p>Departure Date: {selectedFlight.departure_date_time.split('T')[0]}</p>
              <p>Departure Time: {selectedFlight.departure_date_time.split('T')[1]}</p>
              <p>Arrival Date: {selectedFlight.arrival_date_time.split('T')[0]}</p>
              <p>Arrival Time: {selectedFlight.arrival_date_time.split('T')[1]}</p>
              <p>Departure Airport: {selectedFlight.departure_airport.code}</p>
              <p>Arrival Airport: {selectedFlight.arrival_airport.code}</p>
              <p>Max Capacity: {selectedFlight.max_capacity}</p>
              <p>Used Capacity: {selectedFlight.used_capacity[0]}</p>
            </div>
          </InfoWindow>
        )}
      </>

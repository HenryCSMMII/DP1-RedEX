...
function App() {
  ...
  const updateAirportCapacities = (airportCapacities, allShipments, currentDateTime, previousDateTime, processedShipments) => {
    ...
    allShipments.forEach((shipment) => {
      ...
      // Incrementar capacidad al registrar el envío solo una vez
      if (!processedShipments.has(`${shipment.id}-registered`) && registerDateTimeObj.getTime() === currentDateTime.getTime()) {
        if (updatedCapacities[departure_airport]) {
          updatedCapacities[departure_airport].current_capacity += packageQuantity;
          processedShipments.add(`${shipment.id}-registered`); // Marcar el envío como registrado
        } else {
...
const renderMapContent = () => {
  ...
  allShipments.forEach((shipment) => {
    ...
    if (!processedShipments.has(shipment.id)) {
      ...
      if (!processedShipments.has(`${shipment.id}-registered`) && currentDateTime.getTime() === parseISO(shipment.registerDateTime).getTime()) {
